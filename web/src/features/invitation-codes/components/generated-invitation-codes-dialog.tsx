  ...
  return (
    ...
    >
      <div
        ...
        className='max-h-72 min-h-56 overflow-y-auto rounded-lg border'
      >
        {props.codes.map((code) => (
          <button
            ...
            className='group hover:bg-muted/50 flex w-full items-center justify-between gap-3 border-b px-3 py-2.5 text-left font-mono text-sm transition-colors last:border-b-0'
          >
            <span className='truncate'>{code}</span>
            <Copy className='text-muted-foreground size-3.5 shrink-0 opacity-60 transition-opacity group-hover:opacity-100' />
          </button>
        ))}
      </div>
    </Dialog>
