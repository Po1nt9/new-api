...
// deleteUserOAuthBindingsByUserId deletes all OAuth bindings for a user within tx.
// The legacy user_oauth_bindings table is optional (auth identities may live only
// in auth_identities), so skip silently when the table has not been created.
func deleteUserOAuthBindingsByUserId(tx *gorm.DB, userId int) error {
	if !tx.Migrator().HasTable(&UserOAuthBinding{}) {
		return nil
	}
